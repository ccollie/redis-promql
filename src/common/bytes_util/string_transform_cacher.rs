...
fn need_cleanup(last_cleanup_time: &mut Timestamp, current_time: Timestamp) -> bool {
...
mod tests {
    ...
    fn test_need_cleanup() {
        fn f(last_cleanup_time: Timestamp, current_time: Timestamp, result_expected: bool) {
            let mut lct = last_cleanup_time;
            let result = need_cleanup(&mut lct, current_time);
            assert_eq!(result, result_expected,
                       "unexpected result for needCleanup({}, {}); got {}; want {}", last_cleanup_time, current_time, result, result_expected);
            if result {
                assert_eq!(lct, current_time, "unexpected value for lct; got {}; want current_time={}", lct, current_time)
            } else {
                assert_eq!(lct, last_cleanup_time, "unexpected value for lct; got {}; want last_cleanup_time={}", lct, last_cleanup_time);
            }
        }
        f(0, 0, false);
        f(0, 61, false);
