...
use crate::storage::gorilla::NomBitInput;
use nom::{
    bytes,
...
use rusty_chunkenc::uvarint::read_uvarint;
use rusty_chunkenc::varbit_ts::read_varbit_ts;
use rusty_chunkenc::varbit_xor::read_varbit_xor;
use rusty_chunkenc::varint::read_varint;
...
impl XORIterator<'_> {
    ...
    fn read_second_sample(&mut self) -> TsdbResult<Sample> {
        ...
        self.timestamp += i64::try_from(timestamp_delta).map_err(|_| {
            TsdbError::DecodingError("Too Large".to_string())
        })?;
...
fn read_first_sample(input: &[u8]) -> IResult<&[u8], Sample> {
    let (remaining_input, (timestamp, value)) = tuple((read_varint, be_f64))(input)?;
    Ok((remaining_input, Sample { timestamp, value }))
}
