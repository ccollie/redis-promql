...
use crate::joins::HashableIndex;
use serde::Serialize;
use std::convert::TryInto;
use crate::common::types::Timestamp;

/// JoinEngine<TIndex> is responsible for join logic, it consists of two indices. `idx_this` is the LHS and `idx_other` is the RHS. indices are passed by reference
pub struct JoinEngine<'a, TIndex: Serialize + Hash + Clone + Eq + Ord = Timestamp> {
    pub idx_this: &'a HashableIndex<TIndex>,
    pub idx_other: &'a HashableIndex<TIndex>,
}
/// Represents a matched set of indices
pub struct IndexJoinPair {
    pub this_idx: usize,
    pub other_idx: usize,
}

/// Represents a potentially unmatched set of indices
...
pub fn fwd_func(idx: usize, other_len: usize) -> usize {
    if idx >= other_len - 1 {
        other_len - 1
    } else {
        idx + 1
...
impl<'a, TIndex: Serialize + Hash + Clone + Eq + Ord> JoinEngine<'a, TIndex> {
    ...
    fn index_is_same(&self) -> bool {
        #![allow(unused_assignments)] #![allow(unused_mut)] #![allow(clippy::let_and_return)]
        let mut out = false;
        #[cfg(feature = "hash_precompare")]{
    ...
    fn gen_base_lookup(&self, hash_base: &HashableIndex<TIndex>) -> HashMap<TIndex, usize>
    {
        let mut lookup: HashMap<TIndex, usize> = HashMap::with_capacity(hash_base.len());    //reserve to avoid reallocate
        hash_base.iter().enumerate().for_each(|(idx, key)| {
    ...
    /// Hash inner join
    pub fn get_inner_hash_joined_indices(&self) -> Vec<IndexJoinPair>
    {
        if self.index_is_same() {
            ...
            self.idx_this
                .iter()
                .enumerate()
                .map(|(idx, _x)| IndexJoinPair { this_idx: idx, other_idx: idx })
                .collect()
        } else {
    ...
    pub fn get_left_hash_joined_indices(&self) -> Vec<IndexJoinPotentiallyUnmatchedPair>
    {
        if self.index_is_same() {
            //if we are the same just skip this whole thing
    ...
    /// Left Merge Join
    pub fn get_left_merge_joined_indices(&self) -> Vec<IndexJoinPotentiallyUnmatchedPair>
    {
        if self.index_is_same() {
            //if we are the same just skip this whole thing
            self.idx_this.iter().enumerate().map(|(idx, _x)| IndexJoinPotentiallyUnmatchedPair { this_idx: idx, other_idx: Some(idx) }).collect()
    ...
    pub fn get_inner_merge_joined_indices(&self) -> Vec<IndexJoinPair>
    {
        if self.index_is_same() {
            //if we are the same just skip this whole thing
    ...
    pub fn get_asof_merge_joined_indices(&self, compare_func: Option<Box<dyn Fn(&TIndex, &TIndex, &TIndex) -> (cmp::Ordering, i64)>>, other_idx_func: Option<Box<dyn Fn(usize) -> usize>>) -> Vec<IndexJoinPotentiallyUnmatchedPair>
    {
